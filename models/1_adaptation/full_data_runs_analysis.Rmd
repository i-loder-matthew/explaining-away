...
exp_data.optimist = read.csv("~/Documents/research/explaining-away/experiments/02_norming/data/02_norming_cond1-trials.csv")
exp_data.pessimist = read.csv("~/Documents/research/explaining-away/experiments/02_norming/data/02_norming_cond2-trials.csv")

exp_data.pessimist$workerid = exp_data.pessimist$workerid + max(exp_data.optimist$workerid) + 1

exp_data.optimist$condition = "optimist"
exp_data.pessimist$condition = "pessimist"

exp_data = rbind(exp_data.optimist, exp_data.pessimist)
...
d.optimist = convertData("./explaining-away-runs/norming-good-mood/run1_output.json")
d.optimist = reparam(d.optimist)
d.optimist = tidy_sampleList(d.optimist, 1, 1, list())
d.optimist$condition = "optimist"

d.pessimist = convertData("./explaining-away-runs/norming-bad-mood/run1_output.json")
d.pessimist = reparam(d.pessimist)
d.pessimist = tidy_sampleList(d.pessimist, 1, 1, list())
d.pessimist$condition = "pessimist"

d = rbind(d.optimist, d.pessimist)
...
hdi_data.optimist= read.csv(paste("../../models/2_adaptation_model/explaining-away-runs/norming-good-mood/hdi_samples.csv", sep=""))
hdi_data.pessimist = read.csv(paste("../../models/2_adaptation_model/explaining-away-runs/norming-bad-mood/hdi_samples.csv", sep=""))

hdi_data.optimist$condition = "optimist"
hdi_data.pessimist$condition = "pessimist"
hdi_data.all = rbind(hdi_data.optimist, hdi_data.pessimist)
hdi_data.all$condition = factor(hdi_data.all$condition, levels=c("optimist", "pessimist"), ordered = T)
...
d.optimist = convertData("./explaining-away-runs/norming-good-mood/thetas-cost/run1_output.json")
d.optimist = reparam(d.optimist)
d.optimist = tidy_sampleList(d.optimist, 1, 1, list())
d.optimist$condition = "optimist"
