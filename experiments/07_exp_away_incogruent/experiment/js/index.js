...
function make_slides(f) {
    ...
    button: function(response) {
      if (this.step == 1) {
        ...
        if ($("#mood-slider2 .ui-slider-handle").css("display") == "none") {
          ...
          $(".err").hide();
          if (OPTIMIST && ($("#mood-slider2").slider("option", "value") >= previous_mood)) {
            $("#wrong-mood2").show();
            ...
            this.step = 1;
            this.button();
          } else if (PESSIMIST && ($("#mood-slider2").slider("option", "value") <= previous_mood)) {
            $("#wrong-mood2").show();
            this.mood_rating = $("#mood-slider2").slider("option", "value");
            exp.data_mood_ratings.push({
              "type": "mood2",
  ...
  slides.exp_trial = slide({
    ...
    present_handle: function(stim) {
      ...
      utils.make_slider("#prob-slider", this.enable_continue );
      $("#exp_text").html("How likely do you think it is that the man will get <strong>" + stim["seat"] + "</strong>?");
    ...
    button : function(response) {
      if (this.step == 1) {
        ...
        if ($("#mood-slider1 .ui-slider-handle").css("display") == "none") {
          ...
          $(".err").hide();
          if (OPTIMIST && $("#mood-slider1").slider("option", "value") < .5) {
            $("#wrong-mood1").show();
            this.mood_rating = $("#mood-slider1").slider("option", "value");
            exp.data_mood_ratings.push({
              "type": "mood1",
              "mood_rating": this.mood_rating
            });
            this.step = 2;
            this.button();
          } else if (PESSIMIST && $("#mood-slider1").slider("option", "value") > .5) {
            $("#wrong-mood1").show();
            this.mood_rating = $("#mood-slider1").slider("option", "value");
            exp.data_mood_ratings.push({
              "type": "mood1",
  ...
  slides.thanks = slide({
    ...
    start : function() {
      exp.data= {
          ...
          "mood_ratings" : exp.data_mood_ratings,
          "system" : exp.system,
