...
colscale = scale_color_manual(
  ...
  breaks=c("confident-optimist", "cautious-pessimist", "cautious-neutral", "confident-neutral"),
  labels=c("confident-optimist", "cautious-pessimist", "cautious-neutral", "confident-neutral"),
  name="Cond."
...
colscale3 = scale_color_manual(
  values=c("#7CB637", "#4381C1", "#333333", "#999999"),
  breaks=c("cautious-optimist", "confident-pessimist", "cautious-neutral", "confident-neutral"),
  labels=c("cautious-optimist", "confident-pessimist", "cautious-neutral", "confident-neutral"),
  name="Cond."
) 
...
d1_exp$Answer.condition = "confident-optimist"
d2_exp$Answer.condition = "confident-neutral"
d3_exp$Answer.condition = "cautious-pessimist"
d4_exp$Answer.condition = "cautious-neutral"
d5_exp$Answer.condition = "cautious-optimist"
d6_exp$Answer.condition = "confident-pessimist"
...
plot_expression = function(d, colscale, exp = "Exp. 1:") {
  ...
  d$Answer.condition = factor(d$Answer.condition, levels = c("optimist", "pessimist", "cautious-optimist", "confident-pessimist", "cautious-pessimist",  "confident-optimist", "cautious-neutral", "confident-neutral", "neutral"), ordered = TRUE)
  p1 <- ggplot(d, aes(x=percent_window, y=mu, col=Answer.condition, pch=Answer.condition)) + 
    xlab("probability of preferred seat") +
    ylab("mean ratings") +
    geom_errorbar(aes(ymin=mu-ci_low, ymax=mu+ci_high), width=.1) +
    geom_line() +
    geom_point(size=3) +
    guides(pch=guide_legend(title="", title.position = "top" ),
           col=guide_legend(title="",title.position = "top", nrow = 2)) +
    facet_wrap(~modal) +
    theme(legend.position="right") + 
    colscale +
    scale_shape_manual(values=c(16, 17, 15, 3, 16, 17, 15, 3, 2), 
                       breaks=c("optimist", "pessimist", "cautious-optimist", "confident-pessimist", "confident-optimist", "cautious-pessimist", "cautious-neutral", "confident-neutral", "neutral"),
                       labels=c("optimist", "pessimist", "cautious-optimist", "confident-pessimist", "confident-optimist", "cautious-pessimist", "cautious-neutral", "confident-neutral", "neutral"),
                       name="Cond.") +
...
plot.norming = plot_expression(d_means.norming, colscale2, exp="Exp. 1")   +  theme(legend.margin=margin(-20,0,0,0))

ggsave("../papers/openmind/plots/norming.pdf", plot=plot.norming, width=7, height=4, units="cm", scale = 1.75)




plot.exp = plot_expression(d_means.exp, colscale, exp="Exp. 2:") +  theme(legend.margin=margin(-20,0,0,0))

ggsave("../papers/openmind/plots/explaining-away.pdf", plot=plot.exp, width=7, height=4.5, units="cm", scale = 1.75)



plot.exp3= plot_expression(d_means.exp3, colscale3, exp="Exp. 3:")  + theme(legend.margin=margin(-20,0,0,-25))
ggsave("../papers/openmind/plots/incongruent.pdf", plot=plot.exp3, width=7, height=4, units="cm", scale = 1.75)



##### mood ratings

mood1 = (read.csv("06_exp_away/data/06_exp_away_cond1-mood_ratings.csv"))
mood1$Answer.condition = "confident-optimist"
mood2 = (read.csv("04_exp_away/data/04_exp_away_cond2-mood_ratings.csv"))
mood2$Answer.condition = "confident-neutral"
mood3 = (read.csv("04_exp_away/data/04_exp_away_cond3-mood_ratings.csv"))
mood3$Answer.condition = "cautious-pessimist"
mood4 = (read.csv("04_exp_away/data/04_exp_away_cond4-mood_ratings.csv"))
mood4$Answer.condition = "cautious-neutral"
mood5 = read.csv("07_exp_away_incongruent/data/07_exp_away_incongruent_cond5-mood_ratings.csv")
mood5$Answer.condition = "cautious-optimist"
mood6 = read.csv("07_exp_away_incongruent/data/07_exp_away_incongruent_cond6-mood_ratings.csv")
mood6$Answer.condition = "confident-pessimist"
...
mood_plot = mood_by_participant %>% ggplot(aes(x=diff, fill=Answer.condition, color=Answer.condition)) + 
  #geom_histogram(binwidth = 0.05) + 
  geom_density(color="black")  +
  facet_wrap(~Answer.condition, nrow=2) + 
...
ggsave("../papers/cogsci2020-explaining-away//plots/mood-differences-exp2.pdf", plot=mood_plot, width=7, height=5, units="cm", scale = 1.75)
